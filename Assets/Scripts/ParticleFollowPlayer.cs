...
public class ParticleFollowPlayer : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        for (int i = 0; i < particles.GetUpperBound(0); i++)
        {
            ...
            float ForceToAdd = (particles[i].startLifetime - particles[i].remainingLifetime) * (10 * Vector3.Distance(Target.transform.position, particles[i].position));

            //Debug.DrawRay (particles [i].position, (Target.position - particles [i].position).normalized * (ForceToAdd/10));

            particles[i].velocity = (Target.transform.position - particles[i].position).normalized * ForceToAdd;
